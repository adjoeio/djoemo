...
var _ = Describe("Prometheus Metrics", func() {
	...
	Describe("Record", func() {
		It("records counter and histogram with success status", func() {
			...
			// sourceLabel is resolved to "filename:line" of the first caller outside
			// the djoemo library. In tests that caller is the ginkgo test runner.
			Expect(getLabelValue(labels, "source")).To(MatchRegexp(`^.+\.go:\d+$`))
